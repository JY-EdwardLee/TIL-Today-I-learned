...
def create_user(list):
    errored_user = 0 # 에러 유저 0 세팅
    new_user_list = [] # return할 user list
    for user in list:
        result = is_validation(user['blood_group'], user['company'], user['mail'], user['name'], user['website'])
        if result == True:
            ...
            for error in result[1]:
                user[error] = None # 잘못된 필드 None 처리
            errored_user += 1
            new_user_list.append(user)
    print(f'잘못된 데이터로 구성된 유저의 수는 {errored_user}입니다.')
    return new_user_list

        

def is_validation(blood_gorup, company, mail, name, website):
    """
    유저 정보를 검증하고 결과를 반환환
    """
    False_list = [] # False key값 세팅
    validation = []
    # 혈액형 검증
    if blood_gorup in blood_types:
